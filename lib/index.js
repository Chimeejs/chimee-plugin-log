...
chimeeHelper.setLocalStorage('_chimee_t', pageLoadedTimestamp);
var supportLS = chimeeHelper.getLocalStorage('_chimee_t') === pageLoadedTimestamp;
...
var index = popupFactory({
  ...
  beforeCreate: function beforeCreate(_, option) {
    if (chimeeHelper.isUrl(option.logPostUrl)) {
  ...
  create: function create() {
    ...
    if (supportLS) {
      var lsKey = 'auto_on';
      // 下次自动打开日志展示窗口
      var autoOn = chimeeHelper.getLocalStorage(lsKey) === '1';
      $wrap.find('input[type="checkbox"]').on('change', function (e) {
        chimeeHelper.setLocalStorage(lsKey, +this.checked);
      })[0].checked = autoOn;
      // 告诉父组件是否create后开启
      this._hide = !autoOn;
    }
  ...
  methods: {
    writeLog: function writeLog(event) {
      ...
      var logText = this.$txtEl.text() + '\n[' + chimeeHelper.formatDate(new Date(this[logName + 'Timestamp']), 'yyyy-MM-dd hh:mm:ss.i') + '] ' + logName + ' ' + (logData !== undefined ? _JSON$stringify(logData) : '');
    ...
    sendLog: function sendLog(logType, data) {
      ...
      var logData = chimeeHelper.deepAssign(data || {}, this.logBase);

      var logParseFunName = 'send_' + logType + '_log';
      if (this[logParseFunName]) {
        chimeeHelper.deepAssign(logData, this[logParseFunName](logType, logData));
      }
      chimeeHelper.deepAssign(logData, {
        evt: logType,
        _t: +new Date(),
        _r: Math.random()
      });
  ...
  events: chimeeHelper.deepAssign(defaultEvents, {
  ...
  data: {
    ...
    logBase: {
      uid: function (uid) {
        if (!uid) {
          uid = chimeeHelper.uuid();
          chimeeHelper.setLocalStorage('_chimee_uid', uid);
        }
        return uid;
      }(chimeeHelper.getLocalStorage('_chimee_uid')),
      ref: location.href
    }
  }
});
